...
\subsection{Definition of function and related concepts}
\begin{definition}
...
\begin{example}
    Let $f:\Q\to\Q$, $f\left( x \right) = 2x$ be a bijective function. To compute it's inverse suppose $x = f^{-1}\left( y \right) $, then $f\left( f^{-1}\left( y \right)  \right) = 2f^{-1}\left( y \right) $, and therefore $f^{-1}\left( y \right) = \frac{y}{2}$.
\end{example}
\begin{remark}
